...
If you do not want to install git, you can instead download [master.zip](https://github.com/ageron/tf2_course/archive/master.zip), unzip it, rename the resulting directory to `tf2_course` and move it to your development directory.
...
## Create the `tf2c` Environment
Next, make sure you're in the `tf2_course` directory and run the following command. It will create a new `conda` environment containing every library you will need to run all the notebooks (by default, the environment will be named `tf2c`, but you can choose another name using the `-n` option):
...
## Start Jupyter
You're almost there! You just need to register the `tf2c` conda environment to Jupyter. The notebooks in this project will default to the environment named `python3`, so it's best to register this environment using the name `python3` (if you prefer to use another name, you will have to select it in the "Kernel > Change kernel..." menu in Jupyter every time you open a notebook):
...
I may update the notebooks to fix issues and add support for new libraries. So make sure you update this project if you want the latest version.

For this, open a terminal, and run:

    $ cd $HOME # or whatever development directory you chose earlier
    $ cd tf2_course # go to this project's directory
    $ git pull
...
Then we'll delete this project's `tf2c` environment:
